...
`silver-ui-editor-line` is centered on frontend apps work in Lua. The technical target is: Develop a Lua command-oriented project for editor scenarios with log and snapshot fixtures, replay consistency checks, and bounded memory input sets.
...
The score is deterministic. The current threshold is 155. Risk has a penalty of 6, latency has a penalty of 3, and weight contributes a bonus of 2.
